...
function createFileFromChunks($temp_dir, $fileName, $chunkSize, $totalSize) {
    ...
    if ($total_files * $chunkSize >=  ($totalSize - $chunkSize + 1)) {
        ...
        if (($fp = fopen('../files/'.$_POST['event_id'].'/'.$fileName, 'w')) !== false) {
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    $temp_dir = '../files/'.$_POST['event_id'].'/'.$_GET['flowIdentifier'];
...
if (!empty($_FILES)) foreach ($_FILES as $file) {
    ...
    //$temp_dir = '../files/'.$_POST['event_id'].'/'.$_POST['flowIdentifier'];
    $temp_dir = '../files/'.$_POST['event_id'].'/temp';
    $dest_file = $temp_dir.'/'.$_POST['flowFilename'].'.part'.$_POST['flowChunkNumber'];

    $file_dir = '../files/'.$_POST['event_id'];
